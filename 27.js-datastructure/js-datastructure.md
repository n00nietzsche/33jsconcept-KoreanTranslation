...
`트리`는 링크드 리스트같은 것입니다. 다만, *계층 구조* 로 많은 **자식 노드** 의 참조들을 보관하는 것만 빼면요. 다른 말로는, 하나의 노드는 하나의 부모 노드만 가지고 있습니다. 도큐먼트 오브젝트 모델(DOM)은 이러한 구조입니다. DOM은 `head`와 `body`로 나뉘는 루트인 `html`노드를 갖습니다. `head`와 `body`는 이후에 우리에게 친근한 많은 *html 태그* 로 나뉘게 됩니다. 내부적인, *프로토타입 상속* 과 *결합* 과 함께 리액트 컴포넌트는 트리 구조를 만들어냅니다. 물론, DOM의 메모리 내부적 표현인, 리액트의 **가상 DOM**도 역시 트리구조입니다.

**바이너리 서치 트리**는 특별합니다. 왜냐하면 각각의 노드는 2개의 자식 그 이상을 가질 수 없기 때문입니다. **왼쪽 자식**은 부모와 똑같거나 그보다 작은 값을 가져야만 하고, **오른쪽 자식**은 더 큰 값을 가져야만 합니다. 이러한 방법에 따라 구조화되어 있고, 균형이 맞게 됩니다. 우리는 어떠한 값도 *로그 시간* 내에 *검색* 할 수 있습니다. 왜냐하면 우리가 각 반복마다 값의 반은 무시할 수 있기 때문입니다. *삽입*과 *삭제*도 역시 로그 시간 내에 일어납니다. 게다가, 가장 작은 값과 가장 큰 값을 구하려면 왼쪽 노드의 끝, 오른쪽 노드의 끝을 찾으면 쉽게 찾아집니다. 

트리에 대한 순회는 *가로(horizontal)* 혹은 *세로(vertical)* 로 이뤄질 수 있습니다. **깊이 우선 순회(Depth-First Traversal, DFT)** 는 세로로 이루어집니다. 단순한 반복문으로 구현하는 것보다 재귀문으로 더욱 우아하게 구현할 수 있습니다. 노드는 *pre-order*, *in-order*, *post-order*로 순회될 수 있습니다. 만일 우리가 끝의 노드를 검사하기 전에, 루트를 먼저 탐방할 필요가 있다면, 우리는 *pre-order*를 선택해야 할 것입니다. 하지만, 우리가 루트 노드 전에, 끝의 노드를 먼저 탐방할 필요가 있다면, *post-order*를 선택해야 합니다. 이름이 암시하듯이 *in-order* 는 우리가 노드를 *순서대로* 순회할 수 있게 해줍니다. 이러한 프로퍼티는 바이너리 서치 트리를 더욱 정렬에 최적화 시켜줍니다.
